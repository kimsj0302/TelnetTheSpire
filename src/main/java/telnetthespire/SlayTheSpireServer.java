...
public class  SlayTheSpireServer implements Runnable {
	...
	private ArrayList<AbstractCard> pre;
	private ArrayList<AbstractCard> now;
	...
	for(String command: availableCommands)
	    message += "[" + command + "]";
	sendMessage("명령어: " + message);
    }

	private void cardDiff(){
		HashMap<AbstractCard, Integer> pre_map = new HashMap<>();
		HashMap<AbstractCard, Integer> now_map = new HashMap<>();
		for( AbstractCard c : pre){
			if(pre_map.containsKey(c)){
				pre_map.put(c,pre_map.get(c)+1);
			}
			else{
				pre_map.put(c,0);
			}
		}
		for( AbstractCard c : now){
			if(now_map.containsKey(c)){
				now_map.put(c,now_map.get(c)+1);
			}
			else{
				now_map.put(c,0);
			}
		}
		for(AbstractCard c : pre_map.keySet()){
			if(now_map.containsKey(c)){
				if(pre_map.get(c)> now_map.get(c)){
					sendMessage("다음 카드가 " + (pre_map.get(c)-now_map.get(c)) + "장 없어졌습니다.");
					showCard(c,false,false,false);
				}
				else if(pre_map.get(c)< now_map.get(c)){
					sendMessage("다음 카드가 " + (now_map.get(c)-pre_map.get(c)) + "장 추가되었습니다.");
					showCard(c,false,false,false);
				...
				sendMessage("다음 카드가 " + pre_map.get(c) + "장 없어졌습니다.");
				showCard(c,false,false,false);
			}
		}
		for(AbstractCard c : now_map.keySet()) {
			sendMessage("다음 카드가 " + now_map.get(c) + "장 추가되었습니다.+");
			showCard(c,false,false,false);
		}

		for(AbstractCard c : now)
			pre.add(c.makeStatEquivalentCopy());
	...
	if(!specialCommandsText.equals(""))
	    sendMessage("특별한 명령어: " + specialCommandsText);
    }

    public void displayState() {
	if(state.get("error") != null) {
	    sendMessage(colored("에러: " + state.get("error"), ANSI_RED));
	...
	HashMap<String, Object> gameState = (HashMap<String, Object>) state.get("game_state");
	sendMessage("보스: " + gameState.get("act_boss"));
    }

    public void showHealth(){
	HashMap<String, Object> gameState = (HashMap<String, Object>) state.get("game_state");
	sendMessage("체력: " + gameState.get("current_hp") + "/" + gameState.get("max_hp"));
    }
	...
	HashMap<String, Object> gameState = (HashMap<String, Object>) state.get("game_state");
	sendMessage("골드: " + gameState.get("gold"));
    }
    ...
    public void displayCombat(HashMap<String, Object> gameState, Boolean brief) {
		...
		String playerString = colored((String) gameState.get("class"), ANSI_GREEN) +
			" 체력: " + player.get("current_hp") + "/" + player.get("max_hp") +
			" 방어: " + colored(String.valueOf(player.get("block")), ANSI_GREEN);
		sendMessage(playerString);
		ArrayList<AbstractPower> powers = (ArrayList<AbstractPower>) player.get("powers");
		if (powers.size() > 0) {
			String powersText = "파워: ";
		...
		switch(stance.ID) {
		case "Calm":
			sendMessage("경지: " + colored(stance.name, ANSI_BLUE));
			break;
		case "Wrath":
			sendMessage("경지: " + colored(stance.name, ANSI_RED));
			break;
		case "Divinity":
			sendMessage("경지: " + colored(stance.name, ANSI_PURPLE));
			break;
		}
		...
		for(HashMap<String, Object> monster: monsters) {
			...
			" 체력: " + colored(String.valueOf(monster.get("current_hp")) + "/" + String.valueOf(monster.get("max_hp")), ANSI_RED) +
			" 방어: " + colored(String.valueOf(monster.get("block")), ANSI_GREEN) +
			" 의도: " + colored((String) monster.get("intent"), ANSI_YELLOW);
			if ((int) monster.get("move_adjusted_damage") > 0) {
			expectedDamages += (int) monster.get("move_adjusted_damage") * (int) monster.get("move_hits");
			monsterString += " 데미지: " + monster.get("move_adjusted_damage") +
		...
		sendMessage("예상 데미지: " +  expectedDamagesText);
    ...
    public void displayEvent(HashMap<String, Object> screenState) {
		sendMessage("이벤트: " + screenState.get("event_name"));
	...
	sendMessage("점수: " + screenState.get("score"));
	HashMap<String, Object> gameState = (HashMap<String, Object>) state.get("game_state");
	sendMessage("클래스: " + gameState.get("class"));
	sendMessage("승천: " + gameState.get("ascension_level"));
	sendMessage("시드: " + gameState.get("seed"));
    }
